const wrapIcon = (icon: string): string => `
  <svg
      viewBox="0 0 512 512"
      height="10pt"
      width="10pt"
...
export const stylizeName = (text: string): string => {
...
export const join = (text: string[], delimiter: string): string =>
  text.join(delimiter);

export const makeTitle = (title: string): string =>
  `<span>${title.slice(0, 3)}</span>${title.slice(3)}`;

export const is2Col = (type: string | undefined): boolean => type === "2-col";

const makeIcon = (name: keyof typeof iconMap): string =>
  wrapIcon(iconMap[name]);

export const icons = {
  phone: makeIcon("phone"),
  email: makeIcon("email"),
  website: makeIcon("website"),
